...
public void mosse(int n, char l)
   {
    ...
    while(a !=0 )
    {
        ...
        a = S.nextInt();   //tramite questo comando l'utente può inserire la cordinata orizzontale
        ...
        calcolol = (char) ris;          // qui si concerte nuovamente il calcolo in char
            ...
            if(a>2 || a<-2)
            {                    // controllo per verofocare che l'utente non inserisce un valore superiore di 2 o minore di -2
            ...
            {                                   //questo è il rimo controllo che verifica che la mossa non esce fuori dalla scacchiera orizzontalmente
                System.out.println("sei uscito dai limiti della scacchiera");
            }    
            else
            {   
                // se il valore inserito supera entrambi i controlli allora esce il messaggio della mossa valida e si può proseguire con la messo in verticale
            ...
            if(a==2 || a==-2)
            {   
                //controllo della mossa orizzontale
                ...
                int b = S.nextInt();   //l'utente inserisce di quanto si vuole muovere
                    ...
                    if(b>1 || b<-1 || b==0)
                    {  
                        //controllo per assicurarsi che l'utente non ha inserito un valore che va oltre 1 e -1
                        System.out.println("E' stato inserito un valore errato"); 
                        return;
                    }
                    else if (a==9)
                    ...
                    if(calcolon>8 || calcolon<1)
                    {
                        System.out.println("sei uscito dai limiti della scacchiera");
                        return;
                        //se il calcolo viene minore di 1 o maggiore di 8 vuol dire che l'utente è uscito dai limiti della scacchiera 
                    }
                    else    
                    {      
                        // se non si verifica nessun errore, il programma stampa il risultato
                        String risultato="";
            ...
            if(a==1 || a==-1)
                { 
                    ...
                    int b = S.nextInt(); //l'utente inserisce di quanto si vuole muovere
                        ...
                        if(b>2 || b<-2)
                        {          //controllo per assicurarsi che l'utente non ha inserito un valore che va oltre 2 e -2
                            System.out.println("E' stato inserito un valore errato"); 
                            return;
                        }
                        else if (a==9)
                        ...
                        if(b==1 || b==-1 || b==0)
                        {  
                            //controllo per assicurarsi che l'utente non ha inserito i valori intermedi tra -2 e 2
                            System.out.println("E' stato inserito un valore errato");   
                            return;
                        }
